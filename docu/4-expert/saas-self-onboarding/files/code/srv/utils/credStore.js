...
if (cds.env.profiles.find( p =>  p.includes("hybrid") || p.includes("production"))) {
    try{
        ...
        console.log("[cds] - Credential Store Binding is missing!");
    }
}

class CredStore {

    static async readCredential(namespace, type, name) {
    ...
    static async readCredentialValue(namespace, type, name) {
    ...
    static async #fetchAndDecrypt(privateKey, url, method, headers, body) {
    ...
    static async #fetchAndDecryptValue(privateKey, url, method, headers, body) {
        const result = await fetch(url, {method, headers, body})
            .then(this.#checkStatus)
            .then(response => response.text())
            .then(payload => this.#decryptPayload(privateKey, payload))
    ...
    static #headers(credStore, namespace, init) {
